...
import { ArcAssets, ArcPayments, ArcActivity, ArcNetwork, ArcIntegrations } from '@/pages/dashboard/ArcTools'
...
export default function Dashboard() {
  ...
  const saveDraft = (draft: PaymentDraft) => {
    setDrafts(previous => [{ ...draft, id: crypto.randomUUID(), createdAt: new Date().toISOString() }, ...previous])
    setView('activity')
  }
  ...
  const renderView = () => {
    switch (view) {
      ...
      case 'verifiers': return <Verifiers />
      case 'corrections': return <Corrections />
      case 'api': return <ApiRewards />
      case 'settings': return <Boundary />
      case 'assets': return <ArcAssets />
      case 'payments': return <ArcPayments onSave={saveDraft} />
      case 'activity': return <ArcActivity records={drafts} onPrepare={() => setView('payments')} />
      case 'network': return <ArcNetwork />
      case 'integrations': return <ArcIntegrations />
    ...
    <div className="min-h-[100dvh] bg-bg">
      ...
      <div className="mx-auto flex w-full max-w-[1680px] flex-col lg:flex-row">
        ...
        <aside className="hidden shrink-0 lg:block lg:w-64 xl:w-72">
          <nav aria-label="Dashboard categories" className="sticky top-[72px] flex max-h-[calc(100dvh-72px)] flex-col gap-1 overflow-y-auto border-r border-hairline px-4 py-8">
            ...
            {CATEGORIES.map((c, i) => {
              ...
              return (
                ...
                >
                  <Icon size={15} className={cn('shrink-0 transition-colors', active ? 'text-accent' : 'text-faint group-hover:text-ink-muted')} aria-hidden />
                  <span className="flex-1 text-sm">{c.label}</span>
                  <span className={cn('font-mono text-[9px] tracking-[0.1em]', active ? 'text-accent' : 'text-faint')}>
        ...
        <div className="sticky top-[72px] z-30 border-b border-hairline bg-bg/90 backdrop-blur-md lg:hidden">
          <nav aria-label="Dashboard categories" className="touch-scroll flex gap-1 overflow-x-auto px-4 py-3">
            {CATEGORIES.map((c) => {
              const active = view === c.id
              const Icon = c.icon
              return (
        ...
        <main className="min-w-0 flex-1 px-6 py-8 md:px-10 lg:py-10 xl:px-14">
          <p className="mb-6 rounded-xl border border-hairline bg-surface/50 px-5 py-4 text-xs leading-relaxed text-ink-muted">
            Arc tools use the working testnet read functions and local draft validation. Registry records, organizations, evidence, verifier scores, API keys, rewards, and the status strip remain sample data.
          </p>
          <motion.div
            key={view}
