  ...
  return (
    ...
    >
      <Panel className="group h-full p-5 transition-colors duration-300 hover:border-hairline-strong">
        ...
        <p className="mt-1.5 font-mono text-[10px] uppercase tracking-[0.14em] text-ink-muted">
          {label}
        </p>
  ...
  return (
    <div>
      <ViewHeader
        ...
        actions={
          <ExportJsonButton
            data={{ kpis: DASHBOARD_KPIS, network: NETWORK_STATUS, feed: ANCHOR_FEED }}
            filename="arcwell-overview.json"
          />
        }
      />
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          index="1"
          icon={Anchor}
          label="Anchors submitted"
          value={fmtNum(DASHBOARD_KPIS.anchorsSubmitted)}
          delta="+4.2% / 7d"
          spark={NETWORK_STATUS.slotHistory.slice(0, 16)}
        />
        <KpiCard
          index="2"
          icon={ShieldCheck}
          label="Records verified"
          value={fmtNum(DASHBOARD_KPIS.recordsVerified)}
          delta="+2.8% / 7d"
          spark={NETWORK_STATUS.slotHistory.slice(8, 24)}
        />
        <KpiCard
          index="3"
          icon={Timer}
          label="Avg confirmation"
          value={String(DASHBOARD_KPIS.avgConfirmationMs)}
          suffix="ms"
          delta="-38ms / 24h"
          spark={NETWORK_STATUS.slotHistory.slice(4, 20)}
        />
        <KpiCard
          index="4"
          icon={Users}
          label="Verifier accuracy"
          value={`${DASHBOARD_KPIS.verifierAccuracy}%`}
          delta="+0.3pt / 30d"
          spark={NETWORK_STATUS.slotHistory.slice(12, 28)}
        />
      </div>
        ...
        <Panel className="xl:col-span-7">
          ...
          <div className="p-5">
            ...
            <div className="mt-8 grid grid-cols-2 gap-4 border-t border-hairline pt-6 sm:grid-cols-4">
              ...
              <div>
                <p className="tabular font-mono text-xl text-ink">
              ...
              <div>
                <p className="tabular font-mono text-xl text-amber">
              ...
              <svg viewBox="0 0 300 96" className="w-full" aria-hidden>
                ...
                <polyline
                  points={NETWORK_STATUS.slotHistory
                    .map(
                      (v, i) =>
                        `${(i / (NETWORK_STATUS.slotHistory.length - 1)) * 300},${96 - ((v - 300) / 300) * 90}`,
                    )
                    .join(" ")}
                  fill="none"
                  stroke="var(--lime)"
      ...
      <Panel className="mt-6">
        <PanelHeader
          ...
          actions={
            <div className="flex items-center gap-2">
              <CopyButton
                text={ANCHOR_FEED[0] ? shortHash(ANCHOR_FEED[0].ref, 64) : ""}
                label="Latest ref"
              />
        ...
        {feed.length === 0 ? (
          <p className="px-5 py-10 text-center text-sm text-ink-muted">
            No {feedFilter} events in the current window.
          </p>
        ) : (
