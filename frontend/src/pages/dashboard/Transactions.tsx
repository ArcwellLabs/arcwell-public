...
function TxExpanded({
  ...
  onOpenFull: (tx: TransactionRecord) => void;
}) {
    ...
    <div className="grid grid-cols-1 gap-6 border-t border-hairline bg-bg/40 px-5 py-5 lg:grid-cols-3">
      <div className="lg:col-span-2">
        ...
        <dl className="space-y-2.5 text-sm">
          ...
          <div className="flex flex-wrap justify-between gap-2">
            <dt className="text-faint">Storage URI</dt>
            <dd className="break-all font-mono text-xs text-ink-muted">{tx.storageUri}</dd>
          </div>
          <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      <div>
        ...
        <p className="mt-4 text-[11px] leading-relaxed text-faint">
          Validation results are informational checks with source and time, they are not legal
          certifications.
        </p>
      </div>
    </div>
...
export default function Transactions() {
  ...
  const filtered = useMemo(() => {
    ...
    const rows = TRANSACTIONS.filter((t) => {
      ...
      if (visibility !== "all" && t.visibilityPolicy !== (visibility as VisibilityPolicy))
        return false;
      if (!q) return true;
    ...
    return [...rows].sort((a, b) => {
      if (sortKey === "eventTimestamp")
        return a.eventTimestamp.localeCompare(b.eventTimestamp) * dir;
    ...
    <div>
      ...
      <Panel>
        <PanelHeader
          ...
          meta={`${filtered.length} of ${TRANSACTIONS.length} records`}
        />
        <div className="flex flex-col gap-3 border-b border-hairline px-5 py-4 xl:flex-row xl:items-center xl:justify-between">
          <SearchInput
            ...
            placeholder="Search ID, ref, hash, series…"
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-hairline">
                    <th className="px-4 py-3 text-left font-mono text-[10px] uppercase tracking-[0.14em] text-faint">
                      Record
                    </th>
                    <SortHeader
                      label="Event type"
                    ...
                    <SortHeader
                      label="Timestamp"
                      active={sortKey === "eventTimestamp"}
                      dir={sortDir}
                      onClick={() => toggleSort("eventTimestamp")}
                    />
                    <th className="px-4 py-3 text-left font-mono text-[10px] uppercase tracking-[0.14em] text-faint">
                      Evidence hash
                    </th>
                  ...
                  {visible.map((tx) => {
                    ...
                    return [
                      ...
                      >
                        ...
                        <td className="tabular whitespace-nowrap px-4 py-3.5 font-mono text-xs text-ink-muted">
                          {fmtDate(tx.eventTimestamp)}
                        </td>
                        <td className="px-4 py-3.5">
                        ...
                        <td className="px-4 py-3.5">
                          <ChevronDown
                            size={15}
                            className={`text-faint transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`}
                            aria-hidden
                          />
                        </td>
                      </tr>,
                  ...
                  <li key={tx.id}>
                    ...
                    >
                      ...
                      <p className="mt-1.5 text-sm text-ink-muted">
                        {tx.eventType} · {tx.externalRef}
                      </p>
                      <div className="mt-2 flex items-center justify-between gap-2">
                        <span className="tabular font-mono text-[10px] text-faint">
                          {fmtDate(tx.eventTimestamp)} · slot {fmtNum(tx.chainAnchor.slot)}
                        </span>
                        <ChevronDown
                          size={14}
                          className={`shrink-0 text-faint transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`}
                          aria-hidden
                        />
                      </div>
                    </button>
      ...
      >
        {selected && (
          <div>
            ...
            <dl>
              <FieldRow label="External ref" mono>
              ...
              <FieldRow label="Corrections" mono>
                {String(selected.correctionCount)}
              </FieldRow>
            </dl>
          </div>
        )}
