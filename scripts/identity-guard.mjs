...
for (const role of ['AUTHOR', 'COMMITTER']) {
  ...
  if (!identity.startsWith(`${expectedName} <${expectedEmail}> `)) {
    console.error(`Identity guard: ${role.toLowerCase()} must use the ARCWELL project identity. Run npm run identity:setup.`);
    process.exit(1);
  }
}
...
if (process.argv.includes('--history')) {
  ...
  for (const commit of commits.split('\n')) {
    ...
    if (author !== expectedName || authorEmail !== expectedEmail || committer !== expectedName || committerEmail !== expectedEmail) {
